...
public class Point {
    ...
    public Point(int x, int y) {        //constructor with parameter
        this.x = x;
        this.y = y;
    }

    public int getX() {                 //method with return type without any parameters
        return x;
    }

    public int getY() {                 //method with return type without any parameters
        return y;
    }

    public void setX(int x) {             //No Return type with one parameter method
        this.x = x;
    }

    public void setY(int y) {             //No Return type with one parameter method
        this.y = y;

    }

    public double distance() {           //method with return type without any parameters
        return Math.sqrt((0 - this.getX()) * (0 - this.getX()) + (0 - this.getY()) * (0 - this.getY()));

    }

    public double distance(int x, int y) {      //method with return type with two parameters
        return Math.sqrt((x - this.getX()) * (x - this.getX()) + (y - this.getY()) * (y - this.getY()));
    }

    public double distance(Point second) {      //method with return type with parameters
